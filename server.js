...
// Accept JSON and URL-encoded form data
...
app.post("/", async (req, res) => {
  try {
    ...
    const submission = {
      name: `${parsedData.q11_Name?.first ?? ""} ${parsedData.q11_Name?.last ?? ""}`,
    ...
    if (checkError) {
      ...
      return res.status(500).send("Duplicate check failed.")
    }

    if (existing.length > 0) {
      console.log("⚠️ Duplicate entry detected for:", submission.email)
      return res.status(200).send("Duplicate entry – already exists.")
    }

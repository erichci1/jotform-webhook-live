...
app.post("/", upload, async (req, res) => {
  ...
  // 3f) Update `assessment_taken` = true in profiles
  ...
  if (updateError) {
    console.error("⚠️ Failed to update assessment_taken:", updateError)
    return res.status(500).send("Insert succeeded but profile update failed")
  }

  console.log("✅ Insert succeeded and assessment_taken = true updated")
  res.send("OK")
})
